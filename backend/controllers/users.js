...
const cognitoUserPool = cognitoHandler.getCognitoUserPool(
  cognitoData.userPoolId,
  cognitoData.clientId
);
...
const createUser = async (req = request, res = response) => {
  ...
  const nombreArchivo = `${username.replace(
    /[^\w\s]/gi,
    ""
  )}/profile${extension}`;
  ...
  try {
    ...
    const { url: urlPerfil, nombre } = await uploadFile(
      nombreArchivo,
      picture.data
    );
...
const deleteUser = async (req = request, res = response) => {
  ...
  try {
    ...
    const result = jsonUsers.users.filter((user) => user.id === idUsuario);
    // Now we gonna delete this user and rewrite json file
    jsonUsers.users.pop(result);
    const new_json_users = JSON.stringify(jsonUsers);
    fs.writeFileSync(
      path.resolve(__dirname, "../database/users.json"),
      new_json_users,
